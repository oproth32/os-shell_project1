...
tokenlist **split_by_pipe(const tokenlist *tokens) {
    ...
    for (int i = 0; i < tokens->size; ++i) {
        ...
        // error checking for pipe syntax
        if (strcmp(tok, "|") == 0) {
...
// make a temporary file path for pipeline stages
static int mktemp_path(char *buf, size_t n) {
    ...
    if (fd < 0) 
        return -1;
    close(fd); // eopen with redirect functions as needed
    return 0;
}
...
pid_t exec_pipeline_filebacked_bg(tokenlist **segments) {
    ...
    for (int i = 1; i < n - 1; i++) {
        if (run_stage(segments[i], temps[i - 1], temps[i]) != 0) 
            goto fail;
